...
Code.selectTable = function (tableName, tableComment) {
    ...
    if(tableComment != undefined){
    		...
    		Code.setBizName(tableComment);
    }
    Code.selectAllTemplate();
};
...
Code.generate = function () {
    ...
    this.set('projectPath').set('adminPath').set('author').set('projectPackage').set('corePackage').set('ignoreTabelPrefix').set('bizName').set('moduleName').set('parentMenuName');
 ...
 * 设置表注解为模块名
 */
Code.setBizName = function(tableComment){
	$("#bizName").val(tableComment);
}

Code.selectAllTemplate = function(){
	$("#templateList").find(".list-group-item").each(function(index, item){
		var templateKey = $(item).attr('key');
		Code.switchs[templateKey] = true;
