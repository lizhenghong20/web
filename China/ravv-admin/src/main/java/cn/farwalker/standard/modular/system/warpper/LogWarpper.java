...
public class LogWarpper extends BaseControllerWarpper {
    ...
    public void warpTheMap(Map<String, Object> map) {
        ...
        BigInteger i1 = (BigInteger) map.get("userid");
        Long userid = i1.longValue();
        map.put("userName", ConstantFactory.me().getUserNameById(userid));

        //如果信息过长,则只截取前100位字符串
        if (Tools.string.isNotEmpty(message) && message.length() >= 100) {
            String subMessage = message.substring(0, 100) + "...";
            map.put("message", subMessage);
        }

        //如果信息中包含分割符号;;;   则分割字符串返给前台
        if (Tools.string.isNotEmpty(message) && message.indexOf(Contrast.separator) != -1) {
